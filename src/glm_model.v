...
//============================================================================
// glm_model.v  --  GLM-5.2 FULL FORWARD PASS for ONE token position
//                  at the small-but-faithful slice  (ACCEL_GLM52 §2 / §8)
//----------------------------------------------------------------------------
// FUNCTION  (one decode step, single query token at position `pos`)
//
//    x0   = embed(token_id)                                     // table lookup
//    x_{l+1} = decoder_block( x_l , layer=l , mode_l )   l=0..L-1
//                where mode_l = DENSE   for l <  N_DENSE  (first_k_dense_replace)
//                              = MoE     for l >= N_DENSE
//    xN   = rmsnorm_final( x_L )                                // final RMSNorm
//    logits[V] = W_lm[V,MODEL_DIM] . xN                         // LM head GEMV
//    argmax = arg max_v logits[v]                               // next token
//
//   This module is a PURE ORCHESTRATOR.  It REIMPLEMENTS NO ARITHMETIC and
//   contains NO transformer datapath of its own -- it ORCHESTRATES:
//     * ONE glm_decoder_block (the verified GLM-5.2 layer, dense + MoE modes),
//       walked SERIALLY over the L layers by an internal layer_sequencer FSM
//       (serial reuse of a single block instance => minimal area, deterministic
//       latency; a parallel pipeline of L blocks is a drop-in alternative -- not
//       chosen here, documented below).
//     * ONE rmsnorm_unit for the FINAL RMSNorm (ACCEL_GLM52 #19, reuse of #2).
//     * ONE glm_matmul_pipe (the ULTRA-FMAX pipelined systolic GEMM) used as the
//       LM-head GEMV: C[1,LM_TN] = xN[1,MODEL_DIM] x W_lm[MODEL_DIM,LM_TN],
//       issued over VOCAB/LM_TN output tiles (K = MODEL_DIM reduction per tile).
//   The embedding is a simple table lookup (a small embedding ROM the system /
//   TB loads) read through a combinational PULL (em_req/em_idx -> em_val), the
//   SAME pull discipline every sub-unit + the decoder_block use.
//
//----------------------------------------------------------------------------
// LAYER LOOP  (layer_sequencer)
//   ONE decoder_block instance.  The FSM:
//     * holds the running hidden state x in xcur[] (bf16, MODEL_DIM elts);
//     * for each layer l = 0..L-1 sets db_mode = (l < N_DENSE) ? DENSE : MoE,
//       drives db_layer = l on a per-layer index output so the SYSTEM/TB selects
//       layer l's weights (gamma / attn W_* / kv-cache / router W_g / FFN expert
//       W_*) through the SAME pull ports the decoder_block already exposes -- this
//       module FORWARDS every decoder_block pull straight out, ANNOTATED with
//       db_layer so a weight memory indexed by layer answers the right layer;
//     * pulses db_start with x = xcur, waits db_done, latches y_out -> xcur;
//     * advances l; after the last layer hands xcur to the final RMSNorm.
//
//   DENSE / MoE handling: mode is purely the db_mode bit (0=DENSE, 1=MoE), set
//   from l vs N_DENSE.  The decoder_block internally selects the dense swiglu
//   (INTER_DENSE) or the MoE router + TOPK routed + shared experts (INTER_MOE).
//
//   IndexShare (DSA) handling: the DSA indexer lives INSIDE mla_attn (inside the
//   decoder_block).  index_topk_freq=4 + index_skip_topk_offset=3 => a FRESH
//   index set is computed on layers where (l mod 4 == 3) and REUSED by the next 3
//   layers.  This module exposes that schedule on idx_fresh (1 = compute fresh,
//   0 = reuse the cached window) + idx_win (the share-window id, l>>2), and holds
//   a one-window index cache valid flag.  AT THE SLICE SCALE S<=TOPK the indexer
//   is a structural NO-OP (dense fallback: the selected set is always {0..S-1}),
//   so FRESH and REUSE produce the IDENTICAL index set and the result is
//   bit-exact either way -- the schedule is exposed + sequenced for faithfulness
//   (and to drive a real indexer at full scale) but cannot change the slice
//   numerics.  No separate indexer pass is needed in the dense-fallback slice;
//   the decoder_block runs its internal dense-fallback indexer every layer and
//   idx_fresh/idx_win annotate which layers WOULD recompute vs reuse at scale.
//
//----------------------------------------------------------------------------
// LATENCY  (deterministic, data-independent for given params + S)
//   L_model = L_embed (MODEL_DIM beats load)
//           + L * L_decoder_block(params, S)          // serial block reuse
//           + L_rmsnorm(MODEL_DIM)                     // final norm
//           + LM head: (VOCAB/LM_TN) tiles, each
//                 MODEL_DIM (K stream) + `FP_MAC_LAT + 3*`FP_ADD_LAT + few
//           + VOCAB (argmax scan)
//   Every term is a fixed cycle count -> the whole model exposes a fixed,
//   computable latency.  No data-dependent stall anywhere; sync active-high
//   reset; no latch (every reg written every clock in its block); no comb loop
//   (all feedback rides decoder_block / rmsnorm / matmul_pipe pipeline regs).
//
//----------------------------------------------------------------------------
...
module glm_model #(
    ...
    // ---- embedding table pull (combinational): em_val = Emb[token_id][em_idx] ----
    output wire                          em_req,
    ...
    // ---- per-layer index (annotates ALL decoder_block pulls below) ----
    ...
    // ---- decoder_block RMSNorm gamma pull (per-layer; pre-attn/pre-FFN) ----
    output wire                          gn_req,
    output wire                          gn_which,   // 0=pre-attn, 1=pre-FFN
    output wire [DIMW-1:0]               gn_idx,
    input  wire [15:0]                   gn_val,

    // ---- decoder_block attention weight pull (per-layer) ----
    output wire                          aw_req,
    ...
    // ---- decoder_block attention KV-cache read (per-layer) ----
    output wire                          kc_req,
    ...
    // ---- decoder_block MoE router weight pull (per-layer, W_g column) ----
    output wire                          rw_req,
    output wire [R_KW-1:0]               rw_k,
    input  wire [16*N_EXPERT-1:0]        rw_col,

    // ---- decoder_block FFN expert weight pull (per-layer, qualified) ----
    output wire                          fw_req,
    ...
    // ---- final RMSNorm gamma pull (final-norm learned scale) ----
    output wire                          fn_req,     // need a gamma element
    output wire [DIMW-1:0]               fn_idx,
    input  wire [15:0]                   fn_val,     // gamma_final[fn_idx] (bf16)

    // ---- LM-head weight pull (combinational): lm_col[t] = W_lm[lm_k][vtile*LM_TN+t]
    output wire                          lw_req,
    ...
    //========================================================================
    // ONE decoder_block instance (serially reused across the L layers).
    ...
    //   must answer, so a weight memory indexed by layer feeds the right layer.
    //========================================================================
    reg                       db_start;
    ...
    //========================================================================
    // FINAL RMSNorm (rmsnorm_unit, LEN=MODEL_DIM, LANES=1).  It PULLS x (reduce
    // pass) from xcur, then gamma (normalize pass) from the system via fn_*.
    //========================================================================
    reg              fn_start;
    ...
    //   are aligned on the same beat.  Without this the activation (registered,
    //   lagging) and the weight (combinational off lm_k) are off by one.
    ...
    localparam [3:0]
        ...
        M_LAYER  = 4'd2,    // run decoder_block for current layer
